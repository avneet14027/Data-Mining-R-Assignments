...
calculateMaxGain<-function(targetAttrEntropy,training_set,non_target){
    ...
    for(i in 1:ncol(non_target)){
      ...
      for(j in uqClasses){
        ...
        tempcurrentEntropy<-calculateEntropy(currTrainingset);
        currentEntropy<-currentEntropy+(tempcurrentEntropy*(totalCountAttr/totalCountAll));
      }
      infoGAIN<-targetAttrEntropy-currentEntropy;
      #print(infoGAIN);
      if(maxGain < infoGAIN){
        maxGain<-infoGAIN;
        maxGainSel<-i;
...
calculateEntropy<-function(target){
  ...
  p1<-noCount/totalCount;
  p2<-yesCount/totalCount;
...
decisionTree<-function(non_target,training_set){
    ...
    targetAttrEntropy<-calculateEntropy(training_set);
    ...
    for(j in uqClasses){
      currTrainingset<-training_set[training_set[,colLbl]==j,];
      calcEn<-calculateEntropy(currTrainingset);
